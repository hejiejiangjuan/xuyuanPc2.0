...
export default {
  ...
  methods: {
    getCount() {
      document.getElementById("visitorsFlowrate").innerHTML = "";
      this.tubiao();
    },
    tubiao() {
      var that = this;
      ...
      this.$axios
        .post("/backstage/wxUser/getCount", params)
        .then(
          res => {
              ...
              if (res.data.datas.length > 0) {
                ...
                chart.tooltip({
                  ...
                  htmlContent: function htmlContent(title, items) {
                    var alias = {
                      "新用户数量": title+"<br>新用户数量",
                      "参与活动用户数量": title+"<br>参与活动用户数量",
                      "中将用户数量": title+"<br>中将用户数量"
                    };
